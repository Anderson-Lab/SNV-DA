...
allSNPs <- data.matrix(read.csv(paste("~/repos/SNV-DA/test_data_and_pipeline/", args[1], sep=''), header=F, stringsAsFactors=F,sep=","))
names <- read.csv(paste("~/repos/SNV-DA/test_data_and_pipeline/",args[2],sep=''), header=F, sep=',')
...
for (skip1 in 1:num_class_1){
    ...
    auc_val = as.numeric(ci.auc(roc(resp, sub.class1)))[2]
    print(auc_val)
    AUC = c(AUC, auc_val)    
  ...
  bestM = c(bestM, round(mean(testAmounts[which(AUC == max(AUC))])))
