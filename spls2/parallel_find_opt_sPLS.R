...
testAmounts = seq(as.numeric(args[6]), as.numeric(args[7]), as.numeric(args[8]))
accuracy = c()
...
bestM <- foreach (skip_index=1:nrow(skips), .packages=c('mixOmics', 'pROC')) %dopar% {
   ...
   sub.aucs = c()
	...
	sub.training.data = training.data[,-sub_skip][which(rownames(training.data) %in% keep),]
        splsda.model <- tryCatch(splsda(t(sub.training.data), factor(t(sub.classes)), ncomp=1, keepX=numFeatures),  error = function(e) {NULL})
